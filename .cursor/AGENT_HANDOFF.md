...
|--------------|-------------------|---------------------------|-----------------|---------|
| *(none — just finished)* | Added iOS 26 liquid glass animations: layout transitions, spring pill pulses, bar spring grow, tooltip pop-in, dropdown spring, header entrance, stat tile crossfade | `app/progress-graph.tsx`, `components/ui/LiquidGlassPillGroup.tsx`, `app/(tabs)/(profile)/index.tsx` | — | Feb 26 2026 |
*Just finished: Progress Fitness tiles v2 — unified 6-tile grid (progress/muscles/activity/history/active days/workouts), all tiles same minimalist layout (title+subtitle+chevron), borderRadius hard 38, scale 0.98, subtitle 14px/0.60 opacity, chevron 0.35 opacity, routes verified against actual app router tree. File: `app/(tabs)/(profile)/index.tsx`.*
*Just finished: Apple Fitness layout — Restructured Progress→Fitness to match reference: centered "Fitness" header + date; Graph (full); 2 stat mini-cards (Active days | Workouts) with sub-labels; Workout heatmap with period label; History | Statistics. Stat cards tap to progress-heatmap.*
*Just finished: Polish pass — unified all borders/radii/weights/colors across Fitness flow to Glass tokens. CalendarHeatmap summary pills, tooltips, cell strokes, text hierarchy. FitnessGraphWidget pill constants. HeatmapControls pills. GlassSegmentedControl text tokens. Removed unused imports.*
*Just finished: Fitness card stack — rebuilt Progress→Fitness as 3-card stack: (1) GlassCard wrapping FitnessGraphWidget, (2) GlassCard with Activity title + period/metric GlassPill controls + vertical CalendarHeatmap (fixed 320px height), (3) actions row with History (selected) + Statistics GlassPills. Removed WorkoutProgressWidget and StatisticsButtonWidget from this screen (they still exist as components). All using Glass tokens.*
*Just finished: Liquid Glass UI kit — added Glass tokens to constants/theme.ts (fill, border, shadow, radius tiers 38/20/12, text colors). Created components/ui/GlassCard.tsx (translucent card with LinearGradient inner glow, configurable radius, shadow), components/ui/GlassPill.tsx (pill button with selected/pressed states, small/default sizes), components/ui/GlassSegmentedControl.tsx (spring-animated segmented control with glass styling).*
*Just finished: Progress redesign — removed gender toggle from HeatmapControls (gender read from settings); CalendarHeatmap vertical orientation; statistics.tsx wraps heatmaps in Card gradientFill; workout-save + workout-history use HomeGradientBackground; profile index shows vertical calendar under graph in Fitness; all routes verified clean.*
*Just finished: Premium Heatmaps System — two-tab statistics screen (Calendar | Body). New files: utils/dateBins.ts (calendar grid bins, aggregation, bucketing), components/heatmaps/CalendarHeatmap.tsx (GitHub-style SVG grid, 5-level C6C6C6 ramp, tooltip, legend), components/heatmaps/HeatmapControls.tsx (view/period/metric/gender controls). Modified: utils/workoutMuscles.ts (added workoutsToSetRecordsForRange for arbitrary date ranges), components/MuscleBodyHeatmap.tsx (period + externalGender props, hides week nav in non-week mode, aggregate totalSets), components/DetailedBodyHeatmap.tsx (selectedDay=-1 for aggregate), app/(tabs)/(profile)/statistics.tsx (upgraded to two-tab screen).*
*Just finished: Detailed body heatmap — installed react-native-body-highlighter (MIT, 24+ muscle regions, left/right sides, male/female, front/back). Built components/DetailedBodyHeatmap.tsx wrapper mapping our HeatmapData → library's ExtendedBodyPart[] with 5-level C6C6C6 intensity ramp. Replaced SVG in MuscleBodyHeatmap.tsx (statistics), HeatmapPreviewWidget.tsx (swipeable + side-by-side home). Tap muscle → highlights white. No API key needed — pure SVG.*
*Just finished: Muscle Visualizer API integration — utils/muscleVisualizerApi.ts (client), components/MuscleVisualizerImage.tsx (optional component), wired into MuscleBodyHeatmap.tsx + HeatmapPreviewWidget.tsx (both swipeable and side-by-side). When EXPO_PUBLIC_RAPIDAPI_KEY is set, API images replace SVG; otherwise SVG fallback. docs/MUSCLE_VISUALIZER.md added.*
*Just finished: Heatmap polish — BodyAnatomySvg: more labels (Serratus, Quadriceps, Gastrocnemius, Soleus, Rhomboids), full names (Posterior Deltoid, Extensor Carpi Radialis), label stroke primaryLight+70, fontSize 24, dot r=5. MuscleBodyHeatmap: week navigation (‹ This week / Last week / N weeks ago ›), day strip with Mon–Sun + date, today border highlight, Typography for title/weekRange, technical names in list primaryLight+99.*
