/**
 * List 50 Foundation foods for inspection.
 * Run: npx tsx scripts/list-foundation-sample.ts
...
async function fetchFoundationPage(query: string, pageNumber: number): Promise<{ description: string; fdcId: number }[]> {
...
async function main() {
  ...
  for (const term of SEARCH_TERMS) {
    ...
    try {
      const page = await fetchFoundationPage(term, 1);
  ...
  console.log('Foundation sample (50 entries)\n');
  list.forEach((item, i) => {
